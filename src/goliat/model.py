...
class Generator( object ):
    ...
    def generate_model_base( self, table, columns ):
        ...
        for col in columns:
            ...
            if len( _model_primary_keys ):
                attrType = self._parse_column( columns[col], True )
            else:
                attrType = self._parse_column( columns[col] )
        ...
        if relation != None:
            for field, rel in relation.iteritems():
                if rel['type'] == 'one2one':
                    ...
                    _attributes.append( ( field, 'Reference({0}_id, "{1}.{2}"'.format( field, self._generate_model_name( rel['foreignTable'] ), rel['foreignKey'] ) ) )
                    _relations.append( ( 'application.model.base.{0}Base'.format( self._generate_model_name( rel['foreignTable'] ) ),
                        '{0}Base'.format( self._generate_model_name( rel['foreignTable'] ) ) ) )
                elif rel['type'] == 'many2one':
                    _attributes.append( ( '{0}'.format( field, 'ReferenceSet("{0}.{1}", "{2}.{3})'.format( 
                        self._generate_model_name( table ), rel['localKey'], self._generate_model_name( rel['foreignTable'] ), rel['foreignKey'] ) ) ) )
                    _relations.append( ( 'application.model.base.{0}Base'.format( self._generate_model_name( rel['foreignTable'] ) ),
                        '{0}Base'.format( self._generate_model_name( rel['foreignTable'] ) ) ) )
                    ...
                    reference += '"{0}.{1}", '.format( self._generate_model_name( table ), rel['localKey'] )
                    reference += '"{0}.{1}", '.format( self._generate_model_name( table ) + self._generate_model_name( rel['foreignTable'] ), _new_keys[0] )
                    reference += '"{0}.{1}", '.format( self._generate_model_name( table ) + self._generate_model_name( rel['foreignTable'] ), _new_keys[1] )
                    reference += '"{0}.{1}"'.format( self._generate_model_name( rel['foreignTable'] ), rel['foreignKey'] )
                    reference += ')'
                    _attributes.append( ( '{0}'.format( field ), reference ) )
                    _relations.append( ( 'application.model.base.{0}Base'.format( self._generate_model_name( rel['foreignTable'] ) ),
                        '{0}Base'.format( self._generate_model_name( rel['foreignTable'] ) ) ) )
                    _relations.append( ( 
                        'application.model.relation.{0}'.format( self._generate_model_name( table ) + self._generate_model_name( rel['foreignTable'] ) ),
                        self._generate_model_name( table ) + self._generate_model_name( rel['foreignTable'] )
                    ) )
    ...
    def generate_many_2_many( self, table, columns ):
        ...
        for field, relation in columns['_relation'].iteritems():
            ...
            t = self._mgr.get_sys_domain().get_template( 'tpl/modelrelation.evoque' )
            ...
            if relation.get( 'fields' ) != None:
                for field in relation['fields']:
                    for fname, fvalue in field.iteritems():
                        attrName = fname
                        attrType = self._parse_column( fvalue, True )
                        _attributes.append( ( attrName, attrType ) )
