...
class ConfigManager(Borg):
    ...
    def load_config(self, config_name, file_name, raw=False, config={}):
        if config_name not in self._config_files:
            ...
            if self._config_files[config_name]['file'] != file_name and os.path.exists(file_name):
                self._config_files[config_name]['file'] = file_name
        
        try:
            if raw:
                self._process_raw_config_file(config_name, RawConfigParser(), config)                
            else:                                
                self._process_config_file(config_name, SafeConfigParser(), config)
    ...
    def reload(self, config_name, raw=False):
        if config_name in self._config_files:
            self.load_config(config_name, self._config_files[config_name]['file'],raw)
    ...
    def _process_config_file(self, config_name, cp, config):
        self._config_files[config_name] = Apply(self._config_files[config_name], config)                
        cp.read(self._config_files[config_name]['file'])        
        if 'Goliat' not in cp.sections():
            raise ConfigManagerException('{0} is not a Goliat config file'.format( self._config_files[config_name]['file'] ))                         
        for s in cp.sections():
            if not s in self._config_files[config_name]: self._config_files[config_name][s] = {}
            ...
            for o in rp.options(s):
                ...
                try: 
                    ...
                    try:
                        ...
                        try:
                            self._config_files[config_name][s][o] = rp.getfloat(s, o)
                        except ValueError:
