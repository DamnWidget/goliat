...
# TODO: This file needs a total code refactorization to line length < 79
'''
...
from goliat.cli import Command, build_reverse_map, userquery, userchoice, userinput
...
class CmdCreate(Command):
    ...
    def _write_project_files(self, pr):
        ...
        try:
            ...
            if pr._verbose: print bold('Writing {0} Twisted tac file.'.format(pr.get_name()))
            ...
            os.mkdir('init.d')
            if pr._verbose: print bold('Writing {0} System V init script on local init.d dir.'.format(pr.get_name().lower().replace(' ', '_')))
            ...
            if pr._verbose: print bold('Writing {0} project file.'.format(pr.get_name().lower().replace(' ', '_')+'.cfg'))
...
class Project(object):
    ...
    def build_template_files(self):
        ...
        self._templates['service']=linux.init_file(self._install_paths, self._options)
    ...
    def build_install_paths(self):
        ...
        self._install_paths['share']='/var/www/goliat/app/{0}'.format(self._options['app_name'].lower())
        self._install_paths['service']='/etc/init.d'
        self._install_paths['config']='/etc/goliat/applications/{0}.cfg'.format(self._options['app_name'].lower().replace(' ', '_'))
