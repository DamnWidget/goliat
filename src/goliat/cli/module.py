...
class CmdGenerateModule( Command ):
    ...
    def perform( self, args ):
        ...
        if opts.get( 'model' ) != None:
            ...
            tmodel = gen.create_m( opts['model'], _schema.find_table( opts['model'] ) )
            _module_model_import = 'from application.model.{0} import {1}'.format( gen._generate_model_name( opts['model'] ), gen._generateModelName( opts['model'] ) )
